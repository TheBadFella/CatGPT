...
| `CATGPT_API_URL` | _empty_ | Legacy alias accepted when `MIMICGATE_API_URL` is unset. |
...
| `CATGPT_API_KEY` | _empty_ | Legacy alias accepted when `MIMICGATE_API_KEY` is unset; also a legacy Compose input for `API_TOKEN`. |
| `OPENAI_API_KEY` | _empty_ | Fallback terminal-client token. |
| `MIMICGATE_MODEL` | `mimicgate-browser` | Default terminal-client model. |
| `CATGPT_MODEL` | _empty_ | Legacy alias accepted when `MIMICGATE_MODEL` is unset. |
